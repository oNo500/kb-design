...
from kb_core.governance.check_term_usage import (
...
class TermUsageTest(unittest.TestCase):
    def setUp(self):
        ...
        self._write(
            "docs/concepts/current.md",
            "# 术语正文 (Term Prose)\n\n"
            "`同形词` 和 **同形词**。\n\n"
            "```markdown\n**围栏同形词**\n```\n",
        )
        self._write(
            "docs/concepts/no-hits.md",
            "普通正文没有指定抽取位置。\n",
        )
        self._write(
    ...
    def test_scans_dynamic_markdown_manifest(self):
        ...
        self.assertEqual(
            ...
            next(
                ...
                if entry["path"] == "docs/glossary.md"
            ),
        )
        self.assertEqual(
            "scanned",
            next(
                entry["scan_state"]
                for entry in result.manifest
                if entry["path"] == "docs/concepts/no-hits.md"
            ),
        )
        self.assertEqual(
            "review",
    ...
    def test_markdown_addition_requires_allowed_write(self):
        ...
        self._write("docs/design/new-diagnostic.md", "# 新增诊断\n")
        subprocess.run(
            ["git", "add", "docs/design/new-diagnostic.md"], cwd=self.root, check=True
        )
        current = current_markdown_manifest(self.root)

        self.assertEqual(
            ["docs/design/new-diagnostic.md"],
            manifest_delta(previous, current, allowed=()),
        )
        self.assertEqual(
            [],
            manifest_delta(
                previous, current, allowed=("docs/design/new-diagnostic.md",)
            ),
        )

        (self.root / "docs/references/reference.md").unlink()
        subprocess.run(
            ["git", "add", "docs/references/reference.md"], cwd=self.root, check=True
        )
        ...
        self.assertEqual(
            ...
            manifest_delta(
                current,
                after_deletion,
                allowed=("docs/design/new-diagnostic.md",),
            ),
        )
    ...
    def test_excluded_contexts_keep_precise_locations(self):
        ...
        current_hits = [
            hit
            for hit in result
            if hit.file == "docs/concepts/current.md" and hit.normalized == "同形词"
        ]
        source_hit = next(
            hit
            for hit in result
            if hit.file == "docs/references/reference.md" and hit.normalized == "同形词"
        )
        history_hit = next(
            hit
            for hit in result
            if hit.file == "docs/decisions/history.md"
    ...
    def test_first_usage_baseline_is_report_only(self):
        ...
        document = {
            ...
            "decisions": [
                {
                    "id": "term-usage-0001",
                    "identity": "docs/concepts/current.md:3:11",
                    "file": "docs/concepts/current.md",
                    "line": 3,
                    "column": 11,
                    "severity": "review",
