...
CURRENT_CUTOVER_PATHS = {
    "scripts/source_model.py": "packages/kb-core/src/kb_core/source_model.py",
    "vocab/generated/source-cutover-payload.json": "build/source-cutover-payload.json",
    "vocab/generated/source-cutover-handoff.json": "build/source-cutover-handoff.json",
    "vocab/topics.yaml": "data/vocab/topics.yaml",
}
...
def current_cutover_paths(value):
    if isinstance(value, str):
        return CURRENT_CUTOVER_PATHS.get(value, value)
    if isinstance(value, list):
        return [current_cutover_paths(item) for item in value]
    if isinstance(value, dict):
        return {key: current_cutover_paths(item) for key, item in value.items()}
    return value
