#!/usr/bin/env python3
"""校验 data/vocab/ 五份词表。规则见 design/topics.md「校验规则」、design/content-model.md。"""
...
if '--record' in sys.argv:
    ...
    with open(lp, 'w') as stream:
        ...
        stream.write('# 运行 uv run kb-core check-topics --record 时追加。\n')
        yaml.safe_dump(hist, stream, allow_unicode=True)
    print('已追加快照到 data/audit/maintenance/signals.yaml')
