...
SCRIPT = REPOSITORY_ROOT / "scripts" / "check-terms.py"
...
""",
        ...
        (self.root / "vocab" / "topics.yaml").write_text(
            """concepts:
...
""",
        ...
        (self.root / "vocab" / "entities.yaml").write_text(
            "entities: []\n", encoding="utf-8"
        )
        (self.root / "vocab" / "types.yaml").write_text(
            "types: []\n", encoding="utf-8"
        )
        (self.root / "concepts" / "fixture.md").write_text(
            """# 标题候选
...
""",
    ...
    def run_script(self):
        completed = subprocess.run(
            [sys.executable, str(self.root / "scripts" / "check-terms.py"), "--all"],
            cwd=self.root,
            check=True,
            capture_output=True,
            text=True,
