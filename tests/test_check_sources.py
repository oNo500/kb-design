...
import scripts.source_model as model
...
class CheckSourcesTests(unittest.TestCase):
    ...
    def issues(self, name, previous=None, allow_legacy=False):
        ...
        return self.require("validate_repository")(
            FIXTURES / name, old, allow_legacy
        )
    ...
    def test_basis_reference_needs_existing_entity(self):
        reference = model.ReferenceUse(
            "basis", "vocab/terms.yaml", "concept:tc-1", "definitions[0].basis[0]",
            {"entity": "missing", "locator": "§ 1"},
        )
        issues = self.require("validate_references")(
            FIXTURES / "reference-contract", [reference]
        )
    ...
    def test_source_and_match_consume_distinct_approved_roles(self):
        issues = self.require("validate_references")(
            FIXTURES / "reference-contract", approved_and_unapproved_role_references()
        )
    ...
    def test_external_group_requires_approved_structure_role(self):
        issues = self.require("validate_references")(
            FIXTURES / "reference-contract", [unapproved_external_group_reference()]
        )
