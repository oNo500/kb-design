...
from kb_obsidian import exporter as export_obsidian
from kb_obsidian.exporter import ExportError, build_content_files
...
def mutated_repository(relative_path, mutate):
    with tempfile.TemporaryDirectory() as temporary:
        ...
        shutil.copytree(ROOT / "data" / "vocab", repo_root / "data" / "vocab")
...
class ExportObsidianTests(unittest.TestCase):
    ...
    def test_output_path_protects_owning_checkout_with_separate_data_root(self):
        """A separate data root must not expose the installed checkout's source directories."""
        with tempfile.TemporaryDirectory() as temporary:
            ...
            source = owning / "apps" / "obsidian" / "src" / "kb_obsidian" / "exporter.py"
            data_root = root / "data-root"
            data_root.mkdir()
            external = root / "external"
            external.mkdir()

            with mock.patch.object(export_obsidian, "__file__", str(source)):
                for output in (
                    owning / "data" / "vocab" / "new-reference",
                    owning / "packages" / "new-reference",
                ):
                    with self.subTest(output=output), self.assertRaisesRegex(
                        ExportError, "protected"
                    ):
                        export_obsidian._validate_output_path(data_root, output)

                for output in (
                    owning / "output" / "reference",
                    external / "reference",
                ):
    ...
    def test_replacement_links_have_one_property_type_across_object_kinds(self):
        ...
        with mutated_repository("data/vocab/topics.yaml", replace_topic) as repo_root:
        ...
        with mutated_repository("data/vocab/entities.yaml", replace_entity) as repo_root:
            properties, _ = split_note(
    ...
    def test_write_export_rejects_nonempty_output_without_changes(self):
        ...
        with mutated_repository("data/vocab/topics.yaml", lambda document: None) as repo_root:
            sentinel = repo_root / "data" / "vocab" / "sentinel.bin"
            sentinel.write_bytes(b"keep-forbidden")
            output = repo_root / "data" / "vocab" / "output"
    ...
    def test_write_export_uses_one_input_snapshot_for_content_and_manifest(self):
        with mutated_repository("data/vocab/topics.yaml", lambda document: None) as repo_root:
            input_path = repo_root / "data/vocab/topics.yaml"
            ...
            output = repo_root / "output" / "export"
            ...
            topics_input = next(
                item for item in manifest["inputs"] if item["path"] == "data/vocab/topics.yaml"
            )
    ...
    def test_cli_argument_errors_use_export_error_contract(self):
        missing_output = subprocess.run(
            [
                sys.executable,
                str(ROOT / "apps/obsidian/src/kb_obsidian/exporter.py"),
                "--repo-root",
                str(ROOT),
            ],
            cwd=ROOT,
        ...
        help_result = subprocess.run(
            [sys.executable, str(ROOT / "apps/obsidian/src/kb_obsidian/exporter.py"), "--help"],
            cwd=ROOT,
            check=False,
            capture_output=True,
            text=True,
    ...
    def test_write_export_rejects_dangling_reference_without_target(self):
        ...
        with mutated_repository("data/vocab/topics.yaml", mutate) as repo_root:
            (repo_root / "output").mkdir()
            output = repo_root / "output" / "export"
            with self.assertRaises(ExportError):
                export_obsidian.write_export(repo_root, output)
