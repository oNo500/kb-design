...
from scripts.governance.term_model import (
    build_terms_schema,
...
from scripts.source_model import SCHEMA_IDS


ROOT = Path(__file__).resolve().parents[2]
FIXTURES = ROOT / "tests" / "fixtures" / "terminology"
SOURCE_ROOT = ROOT / "tests" / "fixtures" / "source-governance" / "valid"
...
class TermModelTests(unittest.TestCase):
    ...
    def test_rejects_duplicate_ids(self):
        ...
        issues = validate_terms(
            parse_terms(value), SOURCE_ROOT, self.topic_ids,
        )
    ...
    def test_subject_fields_resolve_to_current_topics(self):
        ...
        self.assertNotIn(
            ...
            {issue.code for issue in validate_terms(document, SOURCE_ROOT, self.topic_ids)},
        )

        issues = validate_terms(document, SOURCE_ROOT, frozenset())
        self.assertEqual(
            ["TERM_SUBJECT_FIELD_UNKNOWN"],
            [issue.code for issue in issues],
        )
