...
本 target 规定六份正式词表、参考文献目录和经发布门禁启用的术语参考表示，以及内容建立、校验与报告。`kb-obsidian` 从干净设计快照初始化 vault，建立 UUIDv4 `draft` 内容；语言依据按[语言依据结构](../../decisions/structured-label-basis.md)显示等级和真实依据性质，术语输入按[术语发布](../../decisions/term-complete-publication.md)独立校验。
...
按[终端访问](../../decisions/obsidian-agent-entry.md)，新库生成受管理的根 `AGENTS.md`。普通参考刷新不更新旧库规则或用户内容；当前没有可审计查询日志和自动回流接口。
...
- 建立、写作、修改、废弃和引用内容单元；
- 通过受控字段引用正式主题、普通实体、书目文献、来源用途、文档类型、体裁和载体；
- 建立不改变正式结构的用户索引和阅读路径；
...
本 target 引用[内容模型](../model/content-model.md)的内容单元、字段语义、受控值、identifier 和生命周期，引用[主题词表设计](../model/topics.md)、[命名实体词表设计](../model/entities.md)、[参考文献目录](../model/bibliography.md)、[来源用途登记](../model/sources-registry.md)和[层级结构](../model/hierarchy.md)的正式对象、关系、多上位与数组规则，并按[维护](../governance/maintenance.md)保留指标、阈值、动作和决策权边界。
...
受管理表示和派生报告中的所有文件都可以由文件系统工具修改；只有 Obsidian 支持的 Markdown 和 Base 文件可以在 Obsidian 中编辑，普通 JSON manifest 不是 Obsidian 内容格式。对这些文件的修改不回流、不取得项目效力。[视图排序](../../decisions/obsidian-base-sort-preferences.md)允许 Base 表格的合格排序及 YAML 排版作为本地偏好保留，通过发布快照核对其余结构；其他受管理修改仍形成阻断性漂移。派生报告不作为下一次结论的输入。用户文件不属于受管理写集，生成器不得覆盖、移动或删除。
...
`kb/` 保存六份正式词表、参考文献目录及经门禁启用的术语受管理表示；文献进入 `kb/references/`，普通实体保留 `kb/entities/`，其中 `kb/views/` 保存主题、实体、书目和用途的参考 Base。`app/templates/` 保存受管理结构片段。`app/views/` 保存完整应用的 Base 与固定查询入口，至少提供全部内容、draft 内容、active 内容、deprecated 内容、按主题、按实体、按类型与体裁、最近修改、全部正式主题、unassigned 主题、正式实体、正式来源用途和维护报告入口。`app/reports/` 保存诊断和统计，`app/rules/` 保存面向使用者的应用规则说明，`app/manifest.json` 保存完整应用受管理写集的项目清单。`.obsidian/` 只保存最低运行配置和用户后续配置。
...
外部资料必须区分原文摘录和使用者批注。Web Clipper 与人工复制保存的标题、作者、发布日期、捕获日期、URL 和正文只描述捕获事实，不构成 designation、概念对应、来源资格或引用批准。能独立复用的理解另建内容单元；普通笔记引用资料可在正文保存标题与链接，不要求进入设计书目。需要填入受控 references 的文献必须已在参考文献目录登记；引用不自动构成设计贡献或准入。
...
输入只接受现行严格格式。六份词表与参考文献目录均要求 `schema_version: 3`；实体、书目与来源用途文档另要求各自的 `schema`，所有文档保留 `version` 和集合；旧顶层 `url`、`checked`、`role`、旧 basis.entity、紧凑来源引用及未知字段阻断导出；普通实体的限定事实依据允许互斥 url、locator、checked。语言依据继续使用独立的已采纳合同，包括语言 `legacy`，不与外部来源依据混用。
...
| 六份词表与书目的集合 | 对象路径、`kb_object` Text、固定 Tags 和 manifest 数量 | ID 决定路径，集合次序不改变身份；主题顺序用于数组成员次序 |
...
| `source` | `kb_source` Text link 与正文“派生来源”表 | registry 投影为来源用途 Wikilink；正文另保存 item、locator 及每条相邻依据的文献 Wikilink、locator、checked |
...
| 源字段 | 目标落点与类型 | 引用与信息保存 |
|---|---|---|
| `kind`、`form` | `kb_kind`、`kb_form` Text | 原值保存；实体 form 是既定 literal，不伪造载体词表链接 |
| `subjects` | `kb_subjects` List | 真实主题 Wikilinks，原次序保留 |
| `vendor`、`creator` | `kb_vendor` Text link、`kb_creator` List | 只指向普通实体 |
| `replaced_by` | `kb_replaced_by` Text link | 指向本表项目替代对象，空值不推导目标 |
| `version` | `kb_entity_version` Text | 保留既有软件版本值，不改变实体身份 |
| `urls` | 主地址为 `kb_url` Text；正文“来源地址” | 完整保留 role、url、primary；地址不自动取得书目准入或用途 |

### 文献字段

文献页使用 `kb_object: reference`，`kb_id` 保持原文献 ID，路径为 `kb/references/<id>.md`。名称、主题及一般显示字段沿用共同表示，kind 的历史值不自动转为新的文献类型结论。

| 源字段 | 目标落点与类型 | 引用与信息保存 |
|---|---|---|
| `kind`、`tier`、`form` | `kb_kind`、`kb_tier`、`kb_form` Text | 保留原值；不把厂商文档的旧 standard 解释为技术标准 |
| `vendor`、`creator` | `kb_vendor` Text link、`kb_creator` List | 指向已登记普通实体，不自动收录作者 |
| `replaced_by` | `kb_replaced_by` Text link | 只指向书目中的有依据替代，空值不推导目标 |
| `version`、`fixed_sha256` | `kb_reference_version`、`kb_fixed_sha256` Text | 原值保存，不生成固定内容声明；version 为 null 时省略版本 property，正文显示“未登记可核实版本” |
| `source_status` | `kb_source_status` Text 与正文“外部状态” | 有值只表示 current／superseded／withdrawn；缺值时省略 property，正文显示“外部状态未核实”，不推导 current |
| `urls` | 主地址投影为 `kb_url` Text；正文“来源地址” | 完整列 role、url、primary，保留所有非主地址 |
...
| 源字段 | 目标落点与类型 | 引用与信息保存 |
|---|---|---|
| `reference` | `kb_reference` Text link | 指向 `kb/references/<id>.md`，保持用途与文献身份不同 |
...
| `entities` | 可选，零个或多个 | entity reference；只命中普通实体 | `kb_entities` List of Text links，指向 `kb/entities/<id>.md` | 内容涉及正式实体时填写 | 涉及对象改变时人工修改 | 实体涉及计数与实体入口 | 悬空或对象种类错误时无效；缺失或空列表省略；全部目标与顺序须保存；回流未实现 |
| `source` | 可选，零个或一个 | content-unit、entity 或 reference；必须识别目标种类 | `kb_source` Text link；内容指向 `content/<uuidv4>.md`，普通实体指向 `kb/entities/<id>.md`，文献指向 `kb/references/<id>.md` | 内容实际派生自一个内容单元、普通实体或文献时填写 | 派生判断改变时人工修改，不用 `references` 或旧 `origin` 替代 | 按三种对象分别查询实际派生 | 多值、悬空或对象种类错误时无效；目标种类必须保留；回流未实现 |
| `references` | 可选，零个或多个 | reference；命中参考文献目录 | `kb_references` List of Text links，指向 `kb/references/<id>.md` | 内容引用已登记书目文献时填写 | 引用变化时人工修改，不从 `entities` 推导 | 正式引用计数与参考资料入口 | 悬空或对象种类错误时无效；缺失或空列表省略；回流未实现 |
...
正文 Wikilink、Backlinks、用户索引成员、aliases、unlinked mentions 和 Graph edges 都不进入正式计数。它们可以提供人工探索线索，不能替代受控字段语义。实际报告须按当次有效内容计算，历史单条 draft 观察不代替当前数量。内容存在不自动启用正式消费者，也不能把历史空库报告当作当前计数。
...
`apps/obsidian/src/kb_obsidian/exporter.py` 是完整应用的上游参考导出器。它读取六份正式词表、参考文献目录及按门禁启用的术语输入，生成单向参考区：根 `index.md`、`kb/` 和根 `manifest.json`。完整应用的初始化与刷新通过应用包内部调用该导出器；独立参考 artifact 可用 `uv run python -m kb_obsidian.exporter` 生成。导出器不读取或生成 `home.md`、`inbox/`、`sources/`、`content/`、`indexes/`、`attachments/`、`app/` 或 `.obsidian/`，回流仍未实现。
...
现行导出生成 `topics.base`、`entities.base`、`references.base` 和 `sources.base` 四个 `.base` 文件。每个 Base 用 `file.inFolder()` 和 Markdown 扩展名收窄默认数据集，并提供 table view；不使用 formulas、Backlinks 聚合、插件视图或自动写回动作。
...
- 一次读取六份词表、书目及适用术语输入的原始 bytes，同一快照同时用于内容和 manifest；
...
[参考文献分离](../../decisions/source-bibliography-separation.md)不扩大普通 refresh 写集。旧 `kb/entities/` 文献链接、`kb_references` 及相应派生来源，只能通过明确列出的单次迁移调整；正文普通资料链接无需书目准入。迁移清单逐文件说明目标、变换与保护范围，保持内容 UUID、标题、状态和无关正文，迁移前备份并在临时 vault 核对引用。

书目拆分、临时验收、正式库同步和发版分别记录结果，不因新目录、模式或参考导出存在而宣称同步完成。旧格式不进入日常严格读取，不建立长期双格式兼容。

## 待定事项

- 多份真实材料的完整使用验收、正式消费者及维护反馈仍须取得运行证据。
