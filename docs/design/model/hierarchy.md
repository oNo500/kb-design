...
主题词表中的节点都是概念。顶层概念表示本库范围内的学科；一个主题分支由某个顶层概念及其传递下位概念构成。树只表达学科归属和概念层级：顶层以下的起步结构依据现行来源建立，更深层的概念按本库依据建立。

概念记录字段见[主题词表设计](topics.md)，来源身份和用途见[来源名称规范表](sources-registry.md)。理论依据见[词表的层级](../../concepts/vocabulary-hierarchy.md)、[知识体系](../../concepts/body-of-knowledge.md)和[树按学科而非分面的决定](../../decisions/tree-by-discipline.md)。共享来源引用采用严格 v2 合同；[来源收尾](../../decisions/source-completion.md)限定本批引用、数组和历史条目的处置，不表示正式切换、合并或发版。
...
3. 结构来源的用途记录须有经决定批准的 `structure`，同时满足 `mapping`、来源档级和版本限制。`tier`、版本或来源身份不能单独增加结构用途；具体派生与分组仍分别核对依据。
4. 顶层由范围决定。八个顶层是本地建立的范围边界，并分别 `match` 到 GB/T 13745；它们没有实际派生来源，原 `source: self` 以项目判断及审计定位保存。未来新借入按获准来源的对应范围完整建立起步结构，初始状态为 `unassigned`，不要求扩大本库范围。本批已有结构按[来源收尾](../../decisions/source-completion.md)保留现有对象、父项和状态，24 个来源数组的成员与顺序为项目明确选择，不宣称已经完整复制。
...
6. 未来新借入的概念作为上位概念的下位集合原样保留，成员和顺序按来源，不拆、不改，也不为来源结构另配节点标签。本批 24 个既有数组保留当前成员与顺序的决定不自动放宽此规则；SPD-SEP 缺项及部分顺序差异仍待复核，恢复逐序完整复制须另行处理，不由本批新增概念。
7. 复制的记录写实际来源的严格 `source`，并用严格 `match` 指回来源条目。`source` 记录实际派生，`match` 记录概念对应；来源改版后代码可能变化，两者不能互相替代。
...
10. 一个概念的下位来自多个现行结构来源时，每个来源的下位登记为一个数组，外部分组依据使用 `external_group`，只标识外部组，不证明成员完整、顺序一致或每个成员的派生。例如，security 下 ASVS、CWE 和 ATT&CK 分别成组。
...
严格 `source` 含 `registry`、`item`、`locator` 和相邻 `basis`，只用于实际派生；`match` 含 `registry`、`item`、`rel` 和相邻 `basis`，只用于概念映射。`registry` 分别要求经决定批准的 `structure` 或 `mapping` 角色。来源数组以 `external_group` 保存组身份和依据，成员另行核对。旧形状仅保存在 Git 与迁移审计，不提供兼容读取。

[来源收尾](../../decisions/source-completion.md)采纳的 17 条隔离 match 不进入正式引用，原值与未核结论保存于决定隔离清单和 Git 历史；不删除对象、改变标签、状态或其他关系。人才学的历史 6305520 对应须明确标为已删除外部条目。教育心理学对应外部 19070，保留的 education 父项是内部主题关联，不是原文主分类的原样复制。

反向索引只定位当前有效引用，不决定派生或映射；隔离关系不产生正式边，缺边不证明历史没有关系。六份旧迁移账本继续保存原基线库存、分类和阻断，不把旧建议改写为现行结论。本批数据准备及临时验收不建立正式索引、消费者或切换状态。

## 结构来源

以下各节记录现有下位结构所依据的来源，数量和列举描述项目保留的结构，不证明各来源组完整或顺序一致。一个概念下有多个来源时，各来源的下位分别形成数组。当前没有分析层数组。id 使用正式数据已有的描述性 slug，来源代码记在严格派生与映射引用中；本批不形成新 id，引用取值以逐条采纳为准。
...
表中数量为本库保留的知识单元数。SPD 的 7 个现有成员未包含 SPD-SEP；其缺项与部分数组的顺序差异继续审查，不能据此称每个 Knowledge Area 都含 SEP 或逐序完整复制了 CS2023。
...
二级和三级学科清单见 [GB/T 13745 学科分类清单](../../references/gbt-13745.md)。
...
以下预览只说明项目保留的结构，不作为生成输入，不证明来源完整性，也不自行批准来源角色。
...
│   │   [CS2023 AI 12 个知识单元]
│   │   [ATLAS]                                 ← 现有成员保留；用途按决定采纳
│   │   [OWASP LLM Top 10]                      ← 现有成员保留；用途按决定采纳
...
CS2023 的 AI Knowledge Area 有 12 个知识单元：Introduction、Search、KRR、LRR、Probability、ML、NLP、Agents、Planning、Vision、Robotics、SEP。它偏学术，机器人和规划与本库侧重的 LLM 应用工程距离较远，但它是有稳定编号的人工智能结构来源；本批数组的成员和顺序按项目选择保留。尚未用于标引的概念记录是盲区标记，不是负担。id 使用 `artificial-intelligence`；本库侧重体现在内容中，不改变来源体系边界。

LLM 应用相关外部体系继续按现行结构规则处理。ATLAS（2026.07 版，16 个战术，`AML.TA0000`―`AML.TA0015`）和 OWASP LLM Top 10（2025 版，`LLM01:2025`―`LLM10:2025`）分别保留为现行来源数组。NIST AI RMF 的四个功能是治理职能，不是知识划分，当前只作映射；Anthropic 文档随产品迭代，也只作映射。本批用途与逐条引用按来源收尾决定采纳，不扩展这些来源的用途范围。
...
这些 slug、英文原名、代码和 `unassigned` 状态均为现行值；22 个知识单元全部保留，预期多数会长期保持该状态。本批不选择新写法或改变这些成员与顺序，也不以此声明与外部顺序一致。
...
以下条目只记录尚未生效的问题入口。草案中的推荐、方案和名称候选不改变现行范围、实体类别、层级、记录、来源、数据、`Application Profile` 或消费者。

- Web 相关内容失去复制结构后是否够用，观察后决定是否在 Web Platforms 下破例复制 MDN。
- 多层级时，`broader` 列表顺序不赋予含义。[Obsidian 映射](../targets/obsidian.md)的 target binding 保留全部上位链接，不选择主上位，也不把列表首项解释为主上位；只有真实消费者必须选择主上位时，才另行提案并记录决定。
