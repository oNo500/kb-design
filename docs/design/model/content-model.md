...
来源 v2 已实施；术语具体记录与条件式执行已获采纳，编辑源切换仍须完成规定验收。本文继续规定内容字段、值域、标识符、生命周期和应用映射：术语参考消费不新增内容术语字段，不把术语概念身份写入现行受控值，也不改变内容生命周期。载体中文补全仍只改变显示标签与语言依据。
...
术语数据服务于登记生成和只读参考消费，不参与上表的内容受控引用校验。术语 publication 与编辑源切换按[术语实施结构](../../decisions/term-complete-structure.md)的验收条件执行；委托未开放，主题标签和内容字段不改成术语概念身份。仓库正文诊断仍只提供人工复核线索。
...
仓库 Markdown 正文诊断不是外部知识库回流。已采纳的术语应用合同允许在同一干净快照中，完整核对术语记录、依据及发布授权后生成只读术语参考页；正式 publication 须在规定验收通过后实施。该消费不激活词表委托、内容回流或外部正式 vault 写入，不改变本文字段职责。
