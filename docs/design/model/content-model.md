...
内容单元是知识库中可独立引用、可独立标引的最小单位：有标题，只讲一个主题。这是本库内容模型的定义，按[术语具体采纳](../../decisions/term-data-values.md)保留其项目依据；DITA 1.3 §2.2.1 关于 topic 的单主题、标题和复用单位规定提供设计参照，不把本库概念直接宣称为 DITA 外部定义。在 Obsidian 中可以是一篇笔记，在 DITA 中可以对应一个 topic。本文统一称为“内容单元”。
...
`data/vocab/types.yaml` 中每条记录表示一个文档类型概念；`type` 保存概念 id，`label` 提供显示形式。类型按读者要做什么区分，取 Diátaxis 的 4 类，加 DITA 1.3 技术内容中 Diátaxis 没有的 2 类。以下 6 个数据值和来源保持不变。
...
`data/vocab/genres.yaml` 中每条记录表示一个作者立场概念；`genre` 保存概念 id，`label` 提供显示形式。取 IPTC NewsCodes genre 词表的子集，每条 `match` 到 IPTC 的 URI。以下 5 个数据值、定义和本地说明保持不变。
...
`data/vocab/forms.yaml` 中每条记录表示一个载体概念；`form` 保存概念 id。取 IEEE 1484.12.1-2002（LOM）§5.2 Learning Resource Type 的 15 个取值，每条 `match` 到 LOM。LOM 清单混合两个划分特征，按[层级结构](hierarchy.md)的数组规则分开保存。
...
`level` 取修订版教育目标分类的六类认知活动（Anderson 与 Krathwohl 2001，据 Krathwohl 2002 Tables 2–3）。本库将其用于作者自评笔记涉及哪类认知活动，字段可选；不将六类统称为理解深度，不据此证明学习目标已经达成，也不覆盖该分类中的知识维度。名称与范围按[术语具体采纳](../../decisions/term-data-values.md)同步，字段名和既有取值不变。
...
Ahrens 的闪念笔记、文献笔记和永久笔记区分笔记在工作流中的用途，不是本库 draft／active 的一组对应值。`status` 仍表达本库内容的处理状态：未完成、在用或不再维护；`references` 仍记录实际引用。引用非空可以说明笔记使用了文献，不能据此自动判为文献笔记；在用状态也不能自动判为永久笔记。本库不另设笔记用途分类字段，不自动分类，详见[笔记的类型](../../concepts/note-types.md)。
...
2. 优先从概念记录已有且有准入根据的英文 `label` 取值，如 `sql-injection`、`information-retrieval`。英文名称可以沿用来源原名，或按[治理](../governance/governance.md#译名)的译名阶梯取得；第 4 级独立来源与第 5 级模型判断分别记录，均不得改变原概念含义。没有可采用名称时，标识符从来源代码或编号取得，如 GB/T 13745 的 `870-3050`。不用拼音；译名例外不授权 AI 自建概念或替换既有稳定 id。
...
术语模式另为术语概念和术语形式规定 `tc-*`／`tm-*` UUIDv4 身份，本批具体记录按[术语具体采纳](../../decisions/term-data-values.md)保留已分配身份。这些身份不适用于现行主题、实体、类型、体裁、载体、来源或内容单元，不能从标签批量推导，也不因名称相同建立委托。
...
UUIDv4 不从标题、显示文本、alias、文件名、排序位置、时间值、语言或语言模型输出派生，也不提供密码、权限、真实性或完整性。内容字段表、基数、值域和引用关系不因此改变；`source`、`relation` 与 `isReplacedBy` 中的内容引用仍须指向该身份，而标题改变不改变它。概念边界见[标识符](../../concepts/content-identifiers.md)，格式与安全边界见 [RFC 9562 阅读笔记](../../references/rfc-9562.md)。
...
| 字段 | 词表 | 校验 |
|---|---|---|
| `type` | `data/vocab/types.yaml` | 值在表内 |
| `genre` | `data/vocab/genres.yaml` | 值在表内 |
...
3 个状态是本库扩展，没有外部标准直接规定内容单元的生命周期；取值与词表生命周期对齐，依据 Z39.19 §11.3 的新增、修改和废弃。内容单元不是 ISO 15489 意义上的文件，因为它不是业务证据；该标准不直接适用。但其处置原则可以借用：每类对象的保留和销毁要有书面决定和理由（ISO 15489-1 §3.8、§8.5），见[ISO 15489 笔记](../../references/iso-15489.md)。
...
每种应用对应一篇 `docs/design/targets/<应用>.md`。该文件按 [Application Profile](../../concepts/application-profile.md)方法组织应用约束，并引用适用的已采纳决定；它不把 target 的载体能力写回本文。
...
[Obsidian 映射](../targets/obsidian.md)是当前唯一现行 target 文件。`apps/obsidian/` 中的 `kb-obsidian` 工具已经实现新 vault 初始化、内容建立、只读校验和派生报告，并已建立本地持久 vault；2026-09-05 已观察到正式 vault 有一条 draft；内容消费者未正式激活，也不提供可审计检索记录或回流。六份正式词表仍由 `kb-design` 单向导出，应用不反向编辑。

仓库 Markdown 正文诊断不是外部知识库回流。已采纳的术语应用合同允许在同一干净快照中，完整核对术语记录、依据及发布授权后生成只读术语参考页；正式 publication 已在规定验收通过后实施，首批术语参考消费者已启用。该消费不激活词表委托、内容回流或外部正式 vault 写入，不改变本文字段职责。
...
- [ISO 15489-1:2016](https://www.iso.org/standard/62542.html) §3.8 disposition、§8.5 disposition authorities：处置决定的写法，见[笔记](../../references/iso-15489.md)
