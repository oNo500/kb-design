...
顶层以下的起步结构依据[层级结构](hierarchy.md)建立：`computing` 使用 CS2023 的 17 个 Knowledge Area，其余 7 个顶层使用 GB/T 13745 的二级和三级学科。[来源收尾](../../decisions/source-completion.md)保留既有概念身份、标签、层级和状态，按逐条采纳更新来源引用及七项范围说明；24 个既有数组保留成员和顺序，作为项目选择，不再声明逐序完整复制。
...
数组在文件顶部单独登记，保留 id 与 superordinate。外部数组使用 external_group，结构为 registry、item、locator、basis，要求获准 structure 角色；这只证明分组本身，成员的实际派生另以 source 核对。[来源收尾](../../decisions/source-completion.md)批准本批 24 个数组保留现有成员和顺序作为项目选择；SPD-SEP 缺项及顺序差异继续复核，不声称完整复制，不新增概念或更改父项。已批准的分析数组可以使用已登记 characteristic，不能通过迁移新建划分特征。
...
人才学保留既有概念、父项和历史 6305520 对应，依据必须说明该外部条目已删除，不将它表示为现行条目。教育心理学按[来源收尾](../../decisions/source-completion.md)对应外部 19070，education 父项保持为内部主题关联，不宣称复制了原文主分类。ASVS 密码学使用 closeMatch 表示应用安全要求与通用密码学的范围区别，不改变 CS2023 主派生与对应。
...
主题生成器先完成原有生成，再应用采纳并标准化语言依据；载体按现行人工编辑路径应用同一合同。加载器只接受已明确支持的本批授权，并校验输入声明与每条模型授权一致，不能凭一个形似决定文件的路径取得权限。已存在标签只允许名称和依据均相同的幂等应用，不能借同名译法覆盖外部依据。已采纳模型输出必须与对应输入中的标签和依据一致，原英文与历史范围快照必须与原采纳事实相符；缺失、未采纳或过期记录阻断生成及校验。[来源收尾](../../decisions/source-completion.md)采纳的七项来源范围修正以独立决定和准确 before／after 覆盖当前 scope，保留语言采纳 original.scope，不能回写历史快照使校验通过；该明确覆盖不改变语言等级、label 或原采纳日期。旧 Wikidata 的 Q 号、采纳和否决仍保存在 `label-decisions.json`，不被新记录覆盖。旧报告中的建议与辅助材料保留作复核上下文，不冒充已登记的多来源依据。

该文件保存主题和载体已授权形式的生成采纳；glossary 的编辑源切换不改变本文件的所有权，也不将模型译名变成独立术语记录。输出中的模型判断自包含，离线表示不依赖私人报告；完整原名及范围快照由采纳输入承担。以后获得并采纳外部依据时，保留原模型判断与采纳历史，再更新当前依据。
...
术语记录另有三种工作流状态和四种管理状态，其具体值和发布授权分别核对。它们不接管主题 `status`，不把主题 `candidate` 变成术语候选，也不把 `unassigned`、迁移账本中的 `defer` 或任何报告命中机械映射为术语状态。
...
- 主题标签委托仍未开放；术语编辑源及只读消费按已采纳条件式执行，不因同名自动接管主题标签。
