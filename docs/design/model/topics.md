...
来源引用按[来源收尾](../../decisions/source-completion.md)的逐项采纳范围实施。下文按已采纳的[来源字段合同](../../decisions/source-v2-field-contract.md)说明目标记录和严格生成要求；工具只输出并校验新格式，缺少逐字段采纳输入时失败，不退回旧 source 或 match。主题身份、结构、语言采纳和术语未激活边界保持不变。
