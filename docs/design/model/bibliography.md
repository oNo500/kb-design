...
这里分离的是管理职责与准入范围。文献仍可以作为广义实体被描述；本目录管理其设计依据身份，实体词表管理知识内容涉及的产品、组织等对象。关于概念、对象与节点的区别，见[知识图谱](../../concepts/knowledge-graph.md#对象与表示)。
...
- 文献类型尚未完成概念与外部类别的对应核对。既有 `standard` 混有技术标准和厂商文档，`publication` 包含不同形式的材料；目录分离没有解决这项分类问题。重新建模需逐项核对定义、适用对象和迁移去向，并另行采纳；新文献分类须有对应依据，不能仅从旧记录的类值推定。
