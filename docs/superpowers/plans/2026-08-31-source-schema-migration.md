...
- [当前阶段决定](../../../design/decisions/current-stage-scope.md)替代“当前阶段继续建设候选、交付和回滚能力”的安排。当前阶段在必要复核义务完成后停止；完整 candidate、handoff、payload、原子应用和补偿回滚整体进入正式激活阶段。低价值旧断言研究审计关闭，必要高成本证据继续按[证据阶段](../../../design/decisions/evidence-stage-boundary.md)后置。
