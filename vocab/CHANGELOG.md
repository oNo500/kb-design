...
- 2026-09-05：按[批次授权](../design/decisions/structured-label-basis.md#批次授权)补齐 240 条缺失中文，采用第 5 级模型知识并明确外部用法未核实；原有标签、ID、层级、范围、映射与状态保持。语言依据改为结构化等级与记录，历史未采用值保留未重新分级；旧 Wikidata 采纳与否决不覆盖。本条记录修改，不宣告新版本发版。
...
- 2026-09-05：按同批授权补齐 16 条中文标签，采用第 5 级模型知识并保留原英文、范围、分组、ID 和状态；语言依据同步结构化。
...
- 2026-09-01：完成首轮维护审核：复核 2,076 个位置和 308 个报告字符串；73 个名称形式保持开放；17 个候选中 13 个含 `self`；692 个 `unassigned` 中 58 个按现行“有下位”规则执行 L1 批准为 `active`，其余 634 个保持；31 个来源和 5 个来源角色保持。阈值变更为 0；仍开放 13 个归属判断和 4 个候选接受条件。没有 live 来源证据和应用引用，未作来源有效性或候选转正结论；状态批次尚未取得 L3 发版授权，版本仍为 `2026.08`
