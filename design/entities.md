...
与主题词表相同：`candidate` / `active` / `deprecated`，规则见[主题词表设计](topics.md)。没有 `unassigned`——本表不复制任何结构，每条都是按依据（用到了、读到了）本地建立的。
...
作为词表来源使用的标准和知识体系（CS2023、ASVS、GB/T 13745……）同时是本表的实体。两处不重复登记：本表记它是什么（名称、类别、分级、版本、URL、映射）；[来源名称规范表](sources-registry.md)只记它作为词表来源怎么用（`role`、复制位置），并以 `entity` 字段指向本表。
