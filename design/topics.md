...
- id: sql-injection                      # 稳定、小写、连字符；一经引用不改
  ...
  scope: >                               # 范围注释：用于……不用于……
    ...
    参数化查询等防御手段不在此。
  source: self                           # 现行兼容字段；复制记录填实际来源，本地建立为 self
  match:                                 # 现行外部概念映射
...
正式数据继续使用现行紧凑 `basis`、标量 `source` 和 `match.source`。其中 `source` 属于切换前兼容层；新增或解释实际派生时须遵守[来源名称规范表](sources-registry.md)已经收紧的语义，不把普通参考材料、发现线索或断言依据写成实际派生。后置共享 `basis`、`source` 和 `match` 只有在正式迁移、角色批准和消费者切换后才接管相应记录。
