...
状态:草案,依据已落实,待转正。分档方式原为本库自定;其依据是本库获取与信任知识的方式——按 Z39.19 §5.3.5.2,这是组织依据(organizational warrant),标准承认的收词理由之一。分档值作为实体的 `tier` 字段,见[命名实体词表](../entities.md)。
...
写法:对标体系集中登记在 `vocab/sources.yaml`(结构见 [design/topics.md 来源登记表](topics.md));概念文和笔记里引用的文献暂按文末表标注。
