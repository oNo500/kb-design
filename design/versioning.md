...
版本号用日期，不用语义化三段：词表没有“破坏性变更”的概念——id 一经引用不变，`deprecated` 不删，任何版本的引用都能解析——所以主次版本号没有可表达的内容。ATLAS 的内容版本也用 YYYY.MM。

## 发版时机

不是每次改都发版。以下三类变更攒成一批发一版：

- `candidate` 转 `active`，或新建 `active` 概念
- 概念转 `deprecated`
- 借入的来源改版，复核后的增删改

只补 `scope`、`alt`、`match` 不触发发版，随下一版带出。
