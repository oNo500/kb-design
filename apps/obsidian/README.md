...
正式输入包括 `data/vocab/` 下的词表和 `data/references/bibliography.yaml`。刷新核对旧 manifest 的祖先提交与输入哈希，并使用祖先提交的实现验证旧参考区；旧格式不进入当前正式读取入口。

## 来源表示

实体、参考文献与来源用途分别读取 schema_version 3 的正式数据。实体输出到 `kb/entities/`，参考文献输出到 `kb/references/`；`kb_entities` 和 `kb_references` 分别只接受对应目录的目标。`kb_source` 可指向内容单元、普通实体或参考文献。来源用途的 `reference` 指向书目，依据中的直接官方 URL 只适用于获准的普通实体事实字段。旧字段和紧凑来源引用会报错。项目状态与外部状态分别显示，完整保留多地址、复核、观察、用途资格、外部依据、项目判断与历史记录。角色名不代表已获准，决定 ID 不伪装成不存在的笔记链接。语言依据沿用其独立合同。
