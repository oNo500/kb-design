...
class CliTests(unittest.TestCase):
    ...
    def test_composes_init_new_content_validate_and_report_with_one_json_line_each(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(self.design_commit, initialized["design_commit"])
    ...
    def test_omitted_vault_paths_use_the_repository_output_area(self) -> None:
        """A command run from another cwd must still target the owning repository output vault."""
        from kb_obsidian.design_source import DesignSnapshot
        from kb_obsidian.validation import ValidationResult

        snapshot = DesignSnapshot(self.design, self.design_commit, {}, {})
        expected = self.design / "output" / "obsidian"
        with patch("kb_obsidian.cli.default_design_root", return_value=self.design), patch(
            "kb_obsidian.cli.initialize_vault", return_value={"output": str(expected)}
        ) as initialize, patch("kb_obsidian.cli.load_design", return_value=snapshot), patch(
            "kb_obsidian.cli.validate_content", return_value=ValidationResult((), ())
        ) as validate_content, patch("pathlib.Path.cwd", return_value=Path("/unrelated")):
            init_code, _, init_error = self._invoke("init")
            validate_code, _, validate_error = self._invoke("validate")

        self.assertEqual(0, init_code, init_error)
        self.assertEqual(0, validate_code, validate_error)
        initialize.assert_called_once_with(self.design, expected)
        validate_content.assert_called_once_with(snapshot, expected)

    def test_explicit_vault_paths_take_priority_over_the_default(self) -> None:
        """An explicit external path must not be replaced by the repository default."""
        external = Path("/external/vault")
        with patch("kb_obsidian.cli.default_design_root", return_value=self.design), patch(
            "kb_obsidian.cli.initialize_vault", return_value={"output": str(external)}
        ) as initialize:
            code, _, error = self._invoke("init", "--output", str(external))

        self.assertEqual(0, code, error)
        initialize.assert_called_once_with(self.design, external)
    ...
    def test_argument_errors_use_the_one_line_application_error_contract(self) -> None:
        ...
        code, output, error = self._invoke("new-content", "--design-root", "/design")
