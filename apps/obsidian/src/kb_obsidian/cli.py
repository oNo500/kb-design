...
def _parser() -> argparse.ArgumentParser:
    ...
    refresh.add_argument("--vault", type=Path)
    ...
    new_content.add_argument("--vault", type=Path)
    ...
    report.add_argument("--vault", type=Path)
...
def _run(arguments: argparse.Namespace) -> tuple[Mapping[str, object], bool]:
    ...
    default_vault = arguments.design_root / "output" / "obsidian"
    if arguments.command == "init":
        if arguments.output is None:
            arguments.output = default_vault
        return initialize_vault(arguments.design_root, arguments.output), False
    if arguments.vault is None:
        arguments.vault = default_vault
