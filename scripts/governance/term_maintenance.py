...
def _decision_paths(decisions):
    ...
    return {str(value): "design/decisions" for value in decisions}
...
def build_term_reference_index(document, obligations, decisions):
    ...
    for concept_index, concept in enumerate(concepts):
        ...
        entries.append(_index_entry(
            "concept", concept_id, "self", "vocab/terms.yaml",
        ...
        for language_index, language in enumerate(_value(concept, "languages", ())):
            for term_index, term in enumerate(_value(language, "terms", ())):
                ...
                entries.extend((
                    _index_entry(
                        "term", term_id, "self", "vocab/terms.yaml",
                        term_record, "id", term_state,
                    ),
                    _index_entry(
                        "concept", concept_id, "term.concept", "vocab/terms.yaml",
                        term_record, "concept", term_state,
                    ),
                    _index_entry(
                        "term", term_id, "concept.term", "vocab/terms.yaml",
    ...
    for delegation in _value(document, "delegations", ()):
        ...
        file = _value(delegation, "file", "vocab/topics.yaml")
        ...
        if concept_id:
            entries.extend((
                ...
                _index_entry(
                    ...
                    "vocab/terms.yaml", f"concept:{concept_id}", "delegations",
    ...
    for obligation_id, obligation in indexed_obligations.items():
        ...
        entries.append(_index_entry(
            "obligation", obligation_id, "self", "vocab/term-obligations.yaml",
            record, "id", state,
        ))
        for target_index, target in enumerate(_value(obligation, "targets", ())):
            ...
            entries.extend((
                _index_entry(
                    ...
                    "vocab/term-obligations.yaml", record,
        ...
        for path, decision_id in _decision_references(obligation):
            entries.extend(_decision_relation(
                ...
                decision_id, decision_paths, "vocab/term-obligations.yaml",
            ))
        ...
        for index, concept_id in enumerate(_value(output, "concepts", ())):
            entries.extend((
                ...
                _index_entry(
                    ...
                    "vocab/terms.yaml", f"concept:{concept_id}",
...
def _decision_relation(kind, identity, state, record, field_path, decision_id,
                       decision_paths, file="vocab/terms.yaml"):
    decision_file = decision_paths.get(decision_id, "design/decisions")
    return (
        _index_entry(
...
def _target_file(kind, identity):
    if kind == "obligation":
        return "vocab/term-obligations.yaml"
    if kind == "decision":
        return "design/decisions"
    if kind == "generated_output":
        return identity
    return "vocab/terms.yaml"
