...
2. **词表的层级是图谱的骨架**。BT/NT 搬进图谱就是 `subClassOf` / `instanceOf`,其他关系挂在这个骨架上
...
2. **给链接加类型**:frontmatter 里写 `depends_on: [PostgreSQL]` 而不是只在正文 `[[PostgreSQL]]`。这已经是图谱的最小形态——Obsidian 式的双链是一张只有「链接」一种边的图,加类型就是升级
