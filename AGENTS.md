...
- Obsidian 是应用层，也是首个完整应用 target；`kb-design` 的词表参考导出与独立 `kb-obsidian` 应用共同实现该 target，应用实现存在仍不等于消费者激活
- `kb-obsidian` 已实现新 vault 初始化、内容建立、内容校验和派生报告，并已用于建立本地持久 vault 实例；当前没有实际内容数据、运行中的正式消费者、查询日志或回流接口，空库报告也不构成内容使用证据
