...
- 来源与术语基础的原范围见[当前阶段](docs/decisions/current-stage-scope.md)；本次来源迁移已按[来源执行](docs/decisions/source-execution-boundary.md)开放，优先可直接阅读的离线材料，复杂耗时材料标记“不核对”，恢复只用 Git，不实现补偿回滚。仓库当前没有正式来源 v2 数据、正式术语数据、正式义务、正式索引、委托、消费者或切换状态，不得因接口存在而创建或宣称存在
