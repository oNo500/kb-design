...
- [ ] 在审查记录中逐项标出反馈涉及的文件；只允许修改本任务 `Files` 清单中的文件。下列每个修正步骤均须按整节或整篇重写，无修改时执行校验但不创建空提交。
- [ ] 若反馈涉及 `sources/terminology-standards.md`，重新打开本计划列出的术语标准官方页面并核对阅读边界，再整篇重写；运行 `python3 scripts/check-links.py` 与 `git diff --check -- sources/terminology-standards.md`，预期均通过。
- [ ] 完成上一文件的修正校验后运行 `test -z "$(git diff --cached --name-only)"; if ! git diff --quiet HEAD -- sources/terminology-standards.md; then git add sources/terminology-standards.md && test "$(git diff --cached --name-only)" = "sources/terminology-standards.md" && python3 /tmp/kb-extract-glossary-forms.py refresh-consumers /tmp/kb-terminology-glossary-review.tsv /tmp/kb-terminology-glossary-review-refreshed.tsv && mv /tmp/kb-terminology-glossary-review-refreshed.tsv /tmp/kb-terminology-glossary-review.tsv && python3 /tmp/kb-extract-glossary-forms.py guard-removals /tmp/kb-terminology-glossary-review.tsv && git commit -m "[L1] 来源:完成术语标准复核"; fi`；预期退出码为 0 且暂存区为空。
- [ ] 若反馈涉及 `sources/metadata-standards.md`，重新打开 DCMI、PROV-O、SKOS 与 BCP 47 官方文本并核对阅读边界，再整篇重写；运行 `python3 scripts/check-links.py` 与 `git diff --check -- sources/metadata-standards.md`，预期均通过。
- [ ] 完成上一文件的修正校验后运行 `test -z "$(git diff --cached --name-only)"; if ! git diff --quiet HEAD -- sources/metadata-standards.md; then git add sources/metadata-standards.md && test "$(git diff --cached --name-only)" = "sources/metadata-standards.md" && python3 /tmp/kb-extract-glossary-forms.py refresh-consumers /tmp/kb-terminology-glossary-review.tsv /tmp/kb-terminology-glossary-review-refreshed.tsv && mv /tmp/kb-terminology-glossary-review-refreshed.tsv /tmp/kb-terminology-glossary-review.tsv && python3 /tmp/kb-extract-glossary-forms.py guard-removals /tmp/kb-terminology-glossary-review.tsv && git commit -m "[L1] 来源:完成元数据标准复核"; fi`；预期退出码为 0 且暂存区为空。
- [ ] 若反馈涉及 `sources/iso-25964.md`，按“小节去向”重新核销并整篇重写；运行 `rg -n '可以无损落到 RDF|后继版本将' sources/iso-25964.md`，预期无匹配，再运行 `python3 scripts/check-links.py` 与 `git diff --check -- sources/iso-25964.md`，预期均通过。
- [ ] 完成上一文件的修正校验后运行 `test -z "$(git diff --cached --name-only)"; if ! git diff --quiet HEAD -- sources/iso-25964.md; then git add sources/iso-25964.md && test "$(git diff --cached --name-only)" = "sources/iso-25964.md" && python3 /tmp/kb-extract-glossary-forms.py refresh-consumers /tmp/kb-terminology-glossary-review.tsv /tmp/kb-terminology-glossary-review-refreshed.tsv && mv /tmp/kb-terminology-glossary-review-refreshed.tsv /tmp/kb-terminology-glossary-review.tsv && python3 /tmp/kb-extract-glossary-forms.py guard-removals /tmp/kb-terminology-glossary-review.tsv && git commit -m "[L1] 来源:完成 ISO 25964 复核"; fi`；预期退出码为 0 且暂存区为空。
- [ ] 若反馈涉及 `sources/z39-19.md`，按“小节去向”重新核销并整篇重写，逐项确认条款号存在于已读正文；运行 `python3 scripts/check-links.py` 与 `git diff --check -- sources/z39-19.md`，预期均通过。
- [ ] 完成上一文件的修正校验后运行 `test -z "$(git diff --cached --name-only)"; if ! git diff --quiet HEAD -- sources/z39-19.md; then git add sources/z39-19.md && test "$(git diff --cached --name-only)" = "sources/z39-19.md" && python3 /tmp/kb-extract-glossary-forms.py refresh-consumers /tmp/kb-terminology-glossary-review.tsv /tmp/kb-terminology-glossary-review-refreshed.tsv && mv /tmp/kb-terminology-glossary-review-refreshed.tsv /tmp/kb-terminology-glossary-review.tsv && python3 /tmp/kb-extract-glossary-forms.py guard-removals /tmp/kb-terminology-glossary-review.tsv && git commit -m "[L1] 来源:完成 Z39.19 复核"; fi`；预期退出码为 0 且暂存区为空。
- [ ] 若反馈涉及 `sources/iso-15489.md`，重新核对已读正文并整节重写“适用边界”；运行 `rg -n 'origin|本库.*不删' sources/iso-15489.md`，预期无匹配，再运行 `python3 scripts/check-links.py` 与 `git diff --check -- sources/iso-15489.md`，预期均通过。
- [ ] 完成上一文件的修正校验后运行 `test -z "$(git diff --cached --name-only)"; if ! git diff --quiet HEAD -- sources/iso-15489.md; then git add sources/iso-15489.md && test "$(git diff --cached --name-only)" = "sources/iso-15489.md" && python3 /tmp/kb-extract-glossary-forms.py refresh-consumers /tmp/kb-terminology-glossary-review.tsv /tmp/kb-terminology-glossary-review-refreshed.tsv && mv /tmp/kb-terminology-glossary-review-refreshed.tsv /tmp/kb-terminology-glossary-review.tsv && python3 /tmp/kb-extract-glossary-forms.py guard-removals /tmp/kb-terminology-glossary-review.tsv && git commit -m "[L1] 来源:完成 ISO 15489 复核"; fi`；预期退出码为 0 且暂存区为空。
- [ ] 若反馈涉及 `concepts/terminology-database.md` 或 `concepts/glossary.md`，把前者按整篇、后者按“基本单位”整节作为一个原子修正单元重写；重新运行本计划“数据库概念”中的非基本单位差异、形式集合、来源锚点、术语候选、链接和空白校验，预期全部通过。
- [ ] 完成原子修正单元的校验后运行 `test -z "$(git diff --cached --name-only)"; if ! git diff --quiet HEAD -- concepts/glossary.md concepts/terminology-database.md; then git add concepts/glossary.md concepts/terminology-database.md && test -n "$(git diff --cached --name-only)" && test -z "$(git diff --cached --name-only | rg -v '^(concepts/glossary\.md|concepts/terminology-database\.md)$')" && python3 /tmp/kb-extract-glossary-forms.py refresh-consumers /tmp/kb-terminology-glossary-review.tsv /tmp/kb-terminology-glossary-review-refreshed.tsv && mv /tmp/kb-terminology-glossary-review-refreshed.tsv /tmp/kb-terminology-glossary-review.tsv && python3 /tmp/kb-extract-glossary-forms.py guard-removals /tmp/kb-terminology-glossary-review.tsv && git commit -m "[L2] 概念:完成术语数据库复核"; fi`；预期退出码为 0 且暂存区为空。
- [ ] 若反馈涉及 `concepts/controlled-vocabulary.md`，按“小节去向”整篇重写；运行 `rg -n '^#{2,4} .*[:：]|权威性.*借|PostgreSQL' concepts/controlled-vocabulary.md`，预期无匹配，再运行 `python3 scripts/check-links.py`、`python3 scripts/check-terms.py --all > /tmp/kb-terminology-review-controlled.txt` 与 `git diff --check -- concepts/controlled-vocabulary.md`，逐项复核术语报告差异且其余校验通过。
- [ ] 完成上一文件的修正校验后运行 `test -z "$(git diff --cached --name-only)"; if ! git diff --quiet HEAD -- concepts/controlled-vocabulary.md; then git add concepts/controlled-vocabulary.md && test "$(git diff --cached --name-only)" = "concepts/controlled-vocabulary.md" && python3 /tmp/kb-extract-glossary-forms.py refresh-consumers /tmp/kb-terminology-glossary-review.tsv /tmp/kb-terminology-glossary-review-refreshed.tsv && mv /tmp/kb-terminology-glossary-review-refreshed.tsv /tmp/kb-terminology-glossary-review.tsv && python3 /tmp/kb-extract-glossary-forms.py guard-removals /tmp/kb-terminology-glossary-review.tsv && git commit -m "[L2] 概念:完成受控词表复核"; fi`；预期退出码为 0 且暂存区为空。
- [ ] 若反馈涉及 `concepts/vocabulary-construction.md`，按“小节去向”整篇重写；运行 `rg -n '^#{2,4} .*[:：]|候选词.*unassigned' concepts/vocabulary-construction.md`，预期无匹配，再运行 `python3 scripts/check-links.py`、`python3 scripts/check-terms.py --all > /tmp/kb-terminology-review-construction.txt` 与 `git diff --check -- concepts/vocabulary-construction.md`，逐项复核术语报告差异且其余校验通过。
- [ ] 完成上一文件的修正校验后运行 `test -z "$(git diff --cached --name-only)"; if ! git diff --quiet HEAD -- concepts/vocabulary-construction.md; then git add concepts/vocabulary-construction.md && test "$(git diff --cached --name-only)" = "concepts/vocabulary-construction.md" && python3 /tmp/kb-extract-glossary-forms.py refresh-consumers /tmp/kb-terminology-glossary-review.tsv /tmp/kb-terminology-glossary-review-refreshed.tsv && mv /tmp/kb-terminology-glossary-review-refreshed.tsv /tmp/kb-terminology-glossary-review.tsv && python3 /tmp/kb-extract-glossary-forms.py guard-removals /tmp/kb-terminology-glossary-review.tsv && git commit -m "[L2] 概念:完成词表建设复核"; fi`；预期退出码为 0 且暂存区为空。
- [ ] 若反馈涉及 `concepts/vocabulary-mapping.md`，按“小节去向”整篇重写；运行 `rg -n '权威性借过来|拿不准时用.*closeMatch|映射总是有' concepts/vocabulary-mapping.md`，预期无匹配，再运行 `python3 scripts/check-links.py`、`python3 scripts/check-terms.py --all > /tmp/kb-terminology-review-mapping.txt` 与 `git diff --check -- concepts/vocabulary-mapping.md`，逐项复核术语报告差异且其余校验通过。
- [ ] 完成上一文件的修正校验后运行 `test -z "$(git diff --cached --name-only)"; if ! git diff --quiet HEAD -- concepts/vocabulary-mapping.md; then git add concepts/vocabulary-mapping.md && test "$(git diff --cached --name-only)" = "concepts/vocabulary-mapping.md" && python3 /tmp/kb-extract-glossary-forms.py refresh-consumers /tmp/kb-terminology-glossary-review.tsv /tmp/kb-terminology-glossary-review-refreshed.tsv && mv /tmp/kb-terminology-glossary-review-refreshed.tsv /tmp/kb-terminology-glossary-review.tsv && python3 /tmp/kb-extract-glossary-forms.py guard-removals /tmp/kb-terminology-glossary-review.tsv && git commit -m "[L2] 概念:完成词表映射复核"; fi`；预期退出码为 0 且暂存区为空。
- [ ] 若反馈涉及 `concepts/metadata.md`，按“小节去向”整篇重写并重新核对字符串、DCMI `source`、PROV-O derivation 与本地 `basis` 的边界；运行 `python3 scripts/check-links.py`、`python3 scripts/check-terms.py --all > /tmp/kb-terminology-review-metadata.txt` 与 `git diff --check -- concepts/metadata.md`，逐项复核术语报告差异且其余校验通过。
- [ ] 完成上一文件的修正校验后运行 `test -z "$(git diff --cached --name-only)"; if ! git diff --quiet HEAD -- concepts/metadata.md; then git add concepts/metadata.md && test "$(git diff --cached --name-only)" = "concepts/metadata.md" && python3 /tmp/kb-extract-glossary-forms.py refresh-consumers /tmp/kb-terminology-glossary-review.tsv /tmp/kb-terminology-glossary-review-refreshed.tsv && mv /tmp/kb-terminology-glossary-review-refreshed.tsv /tmp/kb-terminology-glossary-review.tsv && python3 /tmp/kb-extract-glossary-forms.py guard-removals /tmp/kb-terminology-glossary-review.tsv && git commit -m "[L2] 概念:完成元数据复核"; fi`；预期退出码为 0 且暂存区为空。
- [ ] 若反馈涉及 `concepts/vocabulary-hierarchy.md`，按“小节去向”整篇重写；运行 `rg -n '张三' concepts/vocabulary-hierarchy.md`，预期无匹配，再运行 `python3 scripts/check-links.py`、`python3 scripts/check-terms.py --all > /tmp/kb-terminology-review-hierarchy.txt` 与 `git diff --check -- concepts/vocabulary-hierarchy.md`，逐项复核术语报告差异且其余校验通过。
- [ ] 完成上一文件的修正校验后运行 `test -z "$(git diff --cached --name-only)"; if ! git diff --quiet HEAD -- concepts/vocabulary-hierarchy.md; then git add concepts/vocabulary-hierarchy.md && test "$(git diff --cached --name-only)" = "concepts/vocabulary-hierarchy.md" && python3 /tmp/kb-extract-glossary-forms.py refresh-consumers /tmp/kb-terminology-glossary-review.tsv /tmp/kb-terminology-glossary-review-refreshed.tsv && mv /tmp/kb-terminology-glossary-review-refreshed.tsv /tmp/kb-terminology-glossary-review.tsv && python3 /tmp/kb-extract-glossary-forms.py guard-removals /tmp/kb-terminology-glossary-review.tsv && git commit -m "[L2] 概念:完成词表层级复核"; fi`；预期退出码为 0 且暂存区为空。
- [ ] 若反馈涉及 `concepts/classifying-new-subjects.md`，按“小节去向”整篇重写；运行 `rg -n 'origin' concepts/classifying-new-subjects.md`，预期无匹配，再运行 `python3 scripts/check-links.py`、`python3 scripts/check-terms.py --all > /tmp/kb-terminology-review-classifying.txt` 与 `git diff --check -- concepts/classifying-new-subjects.md`，逐项复核术语报告差异且其余校验通过。
- [ ] 完成上一文件的修正校验后运行 `test -z "$(git diff --cached --name-only)"; if ! git diff --quiet HEAD -- concepts/classifying-new-subjects.md; then git add concepts/classifying-new-subjects.md && test "$(git diff --cached --name-only)" = "concepts/classifying-new-subjects.md" && python3 /tmp/kb-extract-glossary-forms.py refresh-consumers /tmp/kb-terminology-glossary-review.tsv /tmp/kb-terminology-glossary-review-refreshed.tsv && mv /tmp/kb-terminology-glossary-review-refreshed.tsv /tmp/kb-terminology-glossary-review.tsv && python3 /tmp/kb-extract-glossary-forms.py guard-removals /tmp/kb-terminology-glossary-review.tsv && git commit -m "[L2] 概念:完成新主题分类复核"; fi`；预期退出码为 0 且暂存区为空。
- [ ] 若反馈涉及 `concepts/body-of-knowledge.md`，按“小节去向”整篇重写；运行 `rg -n '只借.*层' concepts/body-of-knowledge.md`，预期无匹配，再运行 `python3 scripts/check-links.py`、`python3 scripts/check-terms.py --all > /tmp/kb-terminology-review-bok.txt` 与 `git diff --check -- concepts/body-of-knowledge.md`，逐项复核术语报告差异且其余校验通过。
- [ ] 完成上一文件的修正校验后运行 `test -z "$(git diff --cached --name-only)"; if ! git diff --quiet HEAD -- concepts/body-of-knowledge.md; then git add concepts/body-of-knowledge.md && test "$(git diff --cached --name-only)" = "concepts/body-of-knowledge.md" && python3 /tmp/kb-extract-glossary-forms.py refresh-consumers /tmp/kb-terminology-glossary-review.tsv /tmp/kb-terminology-glossary-review-refreshed.tsv && mv /tmp/kb-terminology-glossary-review-refreshed.tsv /tmp/kb-terminology-glossary-review.tsv && python3 /tmp/kb-extract-glossary-forms.py guard-removals /tmp/kb-terminology-glossary-review.tsv && git commit -m "[L2] 概念:完成知识体系复核"; fi`；预期退出码为 0 且暂存区为空。
- [ ] 若反馈涉及 `concepts/governance.md`，按“小节去向”整篇重写；运行 `rg -n '单人加 AI' concepts/governance.md`，预期无匹配，再运行 `python3 scripts/check-links.py`、`python3 scripts/check-terms.py --all > /tmp/kb-terminology-review-governance.txt` 与 `git diff --check -- concepts/governance.md`，逐项复核术语报告差异且其余校验通过。
- [ ] 完成上一文件的修正校验后运行 `test -z "$(git diff --cached --name-only)"; if ! git diff --quiet HEAD -- concepts/governance.md; then git add concepts/governance.md && test "$(git diff --cached --name-only)" = "concepts/governance.md" && python3 /tmp/kb-extract-glossary-forms.py refresh-consumers /tmp/kb-terminology-glossary-review.tsv /tmp/kb-terminology-glossary-review-refreshed.tsv && mv /tmp/kb-terminology-glossary-review-refreshed.tsv /tmp/kb-terminology-glossary-review.tsv && python3 /tmp/kb-extract-glossary-forms.py guard-removals /tmp/kb-terminology-glossary-review.tsv && git commit -m "[L2] 概念:完成治理概念复核"; fi`；预期退出码为 0 且暂存区为空。
- [ ] 若反馈涉及 `concepts/README.md`，按“小节去向”整篇重写并逐项核对索引链接与文章标题；运行 `python3 scripts/check-links.py`、`python3 scripts/check-terms.py --all > /tmp/kb-terminology-review-index.txt` 与 `git diff --check -- concepts/README.md`，逐项复核术语报告差异且其余校验通过。
- [ ] 完成上一文件的修正校验后运行 `test -z "$(git diff --cached --name-only)"; if ! git diff --quiet HEAD -- concepts/README.md; then git add concepts/README.md && test "$(git diff --cached --name-only)" = "concepts/README.md" && python3 /tmp/kb-extract-glossary-forms.py refresh-consumers /tmp/kb-terminology-glossary-review.tsv /tmp/kb-terminology-glossary-review-refreshed.tsv && mv /tmp/kb-terminology-glossary-review-refreshed.tsv /tmp/kb-terminology-glossary-review.tsv && python3 /tmp/kb-extract-glossary-forms.py guard-removals /tmp/kb-terminology-glossary-review.tsv && git commit -m "[L2] 概念:完成概念索引复核"; fi`；预期退出码为 0 且暂存区为空。
- [ ] 确认审查反馈全部关闭后，从下一项开始执行最终验收。任何一项失败时，返回上面与目标文件对应的“重写、任务校验、精确暂存、消费者刷新、移除保护、分级提交”步骤；若目标是术语数据库或术语表则返回原子修正单元。修正后重新请求审查，再从本项重新执行全部最终验收。
